#!/usr/bin/env node
// Local runner for the rebrand-portal skill evals.
//
// For one eval: seed its fixture into a fresh, ISOLATED temp workdir *outside*
// the repo — with the rebrand-portal skill copied into a local
...
const EVALS_DIR = dirname(__dirname); // .../rebrand-portal/evals
const SKILL_DIR = dirname(EVALS_DIR); // .../.claude/skills/rebrand-portal
...
I wrote the rebrand-portal skill and I'm running my own local eval harness
...
async function runOnce(evalObj, i) {
  ...
  // Make the rebrand-portal skill resolvable in isolation by copying it
  // into a local .claude/skills/ (verified: `claude -p` finds it there).
  await mkdir(join(workRoot, ".claude", "skills"), { recursive: true });
  await cp(SKILL_DIR, join(workRoot, ".claude", "skills", "rebrand-portal"), {
    recursive: true,
