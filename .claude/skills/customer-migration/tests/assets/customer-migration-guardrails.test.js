...
const hookPath = join(
  repoRoot,
  '.claude/skills/customer-migration/hooks/guard-da-publish.sh',
);
const tempDirs = [];
...
describe('customer migration publish guard hook', () => {
  it('allows DA writes under the company folder and blocks root writes', () => {
    ...
    const blockedWrapper = runGuard(projectDir, {
      tool_input: {
        command: '.claude/skills/customer-migration/scripts/da/copy-folder.sh org repo other',
      },
    });
  ...
  it('rejects reserved company keys in the DA copy script before token lookup', () => {
    const result = spawnSync(
      'bash',
      [
        join(repoRoot, '.claude/skills/customer-migration/scripts/da/copy-folder.sh'),
        'org',
        'repo',
        'en',
      ],
