...
| `only-da-token-required` | Active setup asks the user for only `DA_TOKEN` from DA Network-tab auth; no manual `HLX_ADMIN_TOKEN`, browser `x-auth-token`, or `/secrets.json` path | Step 4a / I2 | no fixture |
...
| `example-prompts-guide-routing` | User-facing prompt examples require a source site, distinguish full/frontend/assets-later flows, and keep collections attached to asset completion | Entry / README | deterministic checks |
...
| `collections-from-searchable-assets` | Step 6 creates company-scoped collections from already-searchable assets via `.claude/skills/customer-migration/scripts/assets/create-collections.js`; no hand-rolled API, no new creds, no backend boot/deploy | Step 6 | fixture + deterministic checks |
| `collections-auto-after-assets` | Step 6 runs automatically for `full` and `assets-only` flows once assets are searchable; the agent does not stop at asset/search verification | Step 6 | no fixture |
