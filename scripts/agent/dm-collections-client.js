 ...
 * client_credentials token (no new secret), mirroring the exact endpoints, api-keys and
 * headers the worker uses:
 *
 *   - Asset search   POST /adobe/assets/search
 *                    x-api-key: <DM client id>, x-ch-request: search,
 *                    x-polaris-search-provider: 3
 *   - Create/list    POST /adobe/assets/collections
...
export class DmCollectionsClient {
  ...
  async #request(path, { method = 'POST', apiKey, body } = {}) {
  ...
  async createCollection({
    ...
    const res = await this.#request(COLLECTIONS_PATH, {
      apiKey: ADOBE_API_KEY_COLLECTIONS,
      body,
    });
    return res.json();
  }
