/**
 * Controller for the collections step (Step 6 of customer-migration).
 *
 ...
 *
 * Reuses the existing environment exactly like enrich-assets: DM technical-account creds
 * from cloudflare/.secrets and the AEM env id from cloudflare/src/config.js. No new secret,
 * no author writes (collections live on the delivery / Content Hub tier).
 *
...
export async function createCollectionsRun({
  ...
  if (assets.length === 0) {
    log.error(
      ...
      + 'Run enrichment (Step 5, scripts/agent/enrich-assets.js) and confirm assets are '
